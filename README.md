# Простой HTTP/1.0 многопоточный сервер

(Многопоточность реализована используя потоки С++11. Мультиплексирование с помощью структуры pollfd. Парсинг параметров команднной строки реализован при помощи getopt. Для демонизации используется daemon(int nochdir, int noclose) из unistd.h)
